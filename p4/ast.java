...
class DeclListNode extends ASTnode {
    ...
    public List<Sym> analyze(SymTable symTable) throws EmptySymTableException, WrongArgumentException{
        List<Sym> list = new ArrayList<>();
        for(DeclNode myDecl:myDecls){
            list.add(myDecl.analyze(symTable));
        }
        return list;
    }
...
class FormalsListNode extends ASTnode {
    ...
    public List<Sym> analyze(SymTable symTable) throws EmptySymTableException, WrongArgumentException {
        ...
        for (FormalDeclNode fdNode : myFormals) {
            list.add(fdNode.analyze(symTable));
        }
        return list;
    }
...
abstract class DeclNode extends ASTnode {
    abstract public Sym analyze(SymTable symTable) throws EmptySymTableException, WrongArgumentException;
...
class VarDeclNode extends DeclNode {
    ...
    public Sym analyze(SymTable symTable) throws EmptySymTableException, WrongArgumentException {
        sym = symTable.lookupLocal(myId.toString());

        if (sym != null && sym.getKind().equals(Kind.FORMAL))
            return sym;


        if (mySize == NOT_STRUCT)
            myId.analyze(symTable, myType.toString(), Kind.NORMAL);
        else
            myId.analyze(symTable, myType.toString(), Kind.STRUCT_VAR);
...
class FnDeclNode extends DeclNode {
    ...
    public Sym analyze(SymTable symTable) throws EmptySymTableException, WrongArgumentException {
        Sym sym = myId.analyze(symTable, myType.toString(), Kind.FUNCTION);

        if (sym.getKind().equals(Kind.UNDEFINED))
            return sym;
...
class FormalDeclNode extends DeclNode {
    ...
    public Sym analyze(SymTable symTable) throws EmptySymTableException, WrongArgumentException {
        return myId.analyze(symTable, myType.toString(), Kind.FORMAL);
    }
...
class StructDeclNode extends DeclNode {
    ...
    public Sym analyze(SymTable symTable) throws EmptySymTableException, WrongArgumentException {
        Sym sym = myId.analyze(symTable, "struct", Kind.STRUCT_DECL);
...
class IdNode extends ExpNode {
    ...
    public Sym analyze(SymTable symTable, String type, Kind kind) throws EmptySymTableException, WrongArgumentException{
        flag = true;
        boolean pass = true;
        if (!kind.equals(kind.FUNCTION) && type.equals("void")) {
            ErrMsg.fatal(myLineNum, myCharNum, "Non-function declared void");
            pass = false;
        }

        if (kind.equals(kind.STRUCT_VAR) && symTable.lookupGlobal(type) == null) {
        ...
        return pass ? sym : new UndefinedSym();
...
class DotAccessExpNode extends ExpNode {
    ...
    public void unparse(PrintWriter p, int indent) {
        ...
        myId.unparse(p, 0);
		p.print("(");
		p.print(myIdSym.type);
		p.print(")");
    }
    public Sym checkDefinedAndGetSym(SymTable symTable) throws EmptySymTableException {
        myLocSym = myLoc.checkDefinedAndGetSym(symTable);
        if (!myLocSym.getKind().equals(Kind.STRUCT_VAR)) {
...
class CallExpNode extends ExpNode {
    public CallExpNode(IdNode name, ExpListNode elist) {
        ...
        myExpList = elist;
        // TODO add bug
        myCharNum = ((ExpNode)name).myCharNum;
        myLineNum = ((ExpNode)name).myLineNum;
    }
